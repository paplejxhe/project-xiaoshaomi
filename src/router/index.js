...
router.beforeEach(async (to, from, next) => {
    ...
    //所有路由中有detailed的都需要进行验证
    if(toPath.indexOf('/detailed')!=-1){
        //获取detailed的上一级路由，如果与来的路由是一样的就跳转
        let originPath =  toPath.slice(0,toPath.lastIndexOf('/'))
        // console.log('originPath',originPath);
        //如果路由不对劲，留在原处
        if(from.path != originPath){
            next(from.path)
        }
        //如果正确则跳转
        else{
